...
def config_entry(is_sub: bool, bootspec: BootSpec, label: str, time: str) -> str:
    ...
    entry += '  icon /efi/refind/themes/rEFInd-minimal/icons/os_nixos.png\n'
...
def copy_directory(from_path: str, to_path: str):
    ...
    # Mark all files and directories as copied in paths
    for root, dirs, files in os.walk(to_path):
        for name in dirs:
            paths[os.path.join(root, name)] = True
        for name in files:
            paths[os.path.join(root, name)] = True
